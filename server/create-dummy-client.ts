...
async function createDummyClient() {
  try {
    ...
    // 1. Create Organization: CV Digital Kreatif (Tim Startup)
    ...
    // 2. Create Owner User for CV Digital Kreatif
    ...
    // 4. Subscribe to Growth Plan (Rp 299,000/month)
    await db.insert(organizationSubscriptions).values({
      id: "99999999-sub1-1111-2222-333333333333",
      organizationId: org1.id,
    ...
    // 5. Add Multiple Add-Ons for CV Digital Kreatif
    ...
    for (let i = 0; i < addOnSubscriptions.length; i++) {
      ...
      await db.insert(organizationAddOnSubscriptions).values({
        id: `99999999-addon-${i + 1}-${String(i).padStart(8, '0')}`,
        organizationId: org1.id,
    ...
    // 6. Create Second Organization: PT Solusi Tech (Medium Company)
    ...
    // 7. Create Owner for PT Solusi Tech
    ...
    // 8. Subscribe to Scale Plan (Rp 749,000/month) 
    await db.insert(organizationSubscriptions).values({
      id: "88888888-sub2-1111-2222-333333333333",
      organizationId: org2.id,
    ...
    // 9. Add Premium Add-Ons for PT Solusi Tech
    const premiumAddOns = [
      {
    ...
    for (let i = 0; i < premiumAddOns.length; i++) {
      ...
      await db.insert(organizationAddOnSubscriptions).values({
        id: `88888888-addon-${i + 1}-${String(i).padStart(8, '0')}`,
        organizationId: org2.id,
        addOnId: addon.addOnId,
        quantity: addon.quantity,
        monthlyPrice: addon.monthlyPrice,
