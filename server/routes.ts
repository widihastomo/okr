...
import { 
  ...
  insertBudgetEntrySchema, insertInitiativeLessonSchema, insertInitiativeHealthCheckSchema, insertInitiativeExperimentSchema,
  type User
} from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // === INITIATIVE LEARNING & DOCUMENTATION ENDPOINTS ===

  // Budget Entries endpoints
  app.get("/api/initiatives/:initiativeId/budget-entries", async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const budgetEntries = await storage.getBudgetEntries(initiativeId);
  ...
  app.post("/api/initiatives/:initiativeId/budget-entries", requireAuth, async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const currentUser = req.user as User;
      
      const budgetData = insertBudgetEntrySchema.parse({
        ...req.body,
        initiativeId,
        createdBy: currentUser.id
      });
      
      const newBudgetEntry = await storage.createBudgetEntry(budgetData);
  ...
  // Initiative Lessons endpoints
  app.get("/api/initiatives/:initiativeId/lessons", async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const lessons = await storage.getInitiativeLessons(initiativeId);
      res.json(lessons);
    } catch (error) {
      console.error("Error fetching initiative lessons:", error);
      res.status(500).json({ message: "Failed to fetch initiative lessons" });
    }
  });

  app.post("/api/initiatives/:initiativeId/lessons", requireAuth, async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const currentUser = req.user as User;
      
      const lessonData = insertInitiativeLessonSchema.parse({
        ...req.body,
        initiativeId,
        documentedBy: currentUser.id
      });
      
      const newLesson = await storage.createInitiativeLesson(lessonData);
      res.status(201).json(newLesson);
    } catch (error) {
      console.error("Error creating initiative lesson:", error);
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: "Invalid data", errors: error.errors });
      }
      res.status(500).json({ message: "Failed to create initiative lesson" });
    }
  });

  // Initiative Health Checks endpoints
  app.get("/api/initiatives/:initiativeId/health-checks", async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const healthChecks = await storage.getInitiativeHealthChecks(initiativeId);
  ...
  app.post("/api/initiatives/:initiativeId/health-checks", requireAuth, async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const currentUser = req.user as User;
      
      const healthCheckData = insertInitiativeHealthCheckSchema.parse({
        ...req.body,
        initiativeId,
        checkedBy: currentUser.id
      });
      
      const newHealthCheck = await storage.createInitiativeHealthCheck(healthCheckData);
  ...
  // Initiative Experiments endpoints
  app.get("/api/initiatives/:initiativeId/experiments", async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const experiments = await storage.getInitiativeExperiments(initiativeId);
  ...
  app.post("/api/initiatives/:initiativeId/experiments", requireAuth, async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const currentUser = req.user as User;
      
      const experimentData = insertInitiativeExperimentSchema.parse({
        ...req.body,
        initiativeId,
        createdBy: currentUser.id
      });
      
      const newExperiment = await storage.createInitiativeExperiment(experimentData);
  ...
  app.patch("/api/experiments/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updatedExperiment = await storage.updateInitiativeExperiment(id, updateData);
  ...
  // Initiative Analysis & Summary endpoints
  app.get("/api/initiatives/:initiativeId/analysis", async (req, res) => {
    try {
      const { initiativeId } = req.params;
      const analysis = await storage.getInitiativeAnalysis(initiativeId);
      res.json(analysis);
    } catch (error) {
      console.error("Error fetching initiative analysis:", error);
      res.status(500).json({ message: "Failed to fetch initiative analysis" });
    }
  });
