...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/registration/send-verification", async (req, res) => {
    try {
      const { userName, businessName, whatsappNumber, email, password } = req.body;
      ...
      // Store verification data temporarily (in session for now)
  ...
  app.post("/api/registration/verify-email", async (req, res) => {
    try {
      const { verificationCode, email, userRegistrationData } = req.body;
      ...
      // Mark as verified
      req.session.pendingRegistration.isVerified = true;
  ...
  app.post("/api/registration/complete", async (req, res) => {
    try {
      const { userRegistrationData, emailVerificationData, onboardingData } = req.body;
      
      // Get pending registration from session
      const pendingRegistration = req.session.pendingRegistration;
      
      if (!pendingRegistration || !pendingRegistration.isVerified) {
      ...
      const organizationData = {
        ...
        size: "small", // Default for now
        ownerId: null, // Will be set after user creation
        trialEndDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 14 days trial
      ...
      // Create user
      const userData = {
        firstName: pendingRegistration.userName,
        lastName: "",
      ...
      // Create initial cycle based on onboarding data
      const cycleData = {
        title: `${onboardingData.cycleDuration} - ${onboardingData.teamFocus}`,
        description: `Siklus ${onboardingData.cycleDuration} dengan fokus ${onboardingData.teamFocus}`,
      ...
      const objectiveData = {
        ...
        description: onboardingData.objective,
        ownerId: user.id,
        cycleId: cycle.id,
        organizationId: organization.id,
      ...
      // Create key results
      for (const keyResultText of onboardingData.keyResults) {
        if (keyResultText.trim()) {
      ...
      // Set up user reminder config
      await storage.updateUserReminderConfig(user.id, {
        isActive: true,
  ...
  app.post("/api/registration/resend-verification", async (req, res) => {
    try {
      ...
      req.session.pendingRegistration.expiresAt = Date.now() + 300000; // 5 minutes
      
      // Send new verification email
      await emailService.sendVerificationEmail(email, verificationCode, pendingRegistration.userName);
