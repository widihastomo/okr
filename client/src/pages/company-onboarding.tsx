                  ...
                  return (
                    <div className="mt-4 pt-4 border-t border-blue-200">
                      <Label className="text-sm font-medium text-blue-800 mb-3 block">
                        Edit Key Results (opsional):
                      </Label>
                      <div className="space-y-3">
                        {selectedTemplate.keyResults.map((templateKr: any, index: number) => (
                          <div key={index} className="space-y-2">
                            <Label className="text-xs text-blue-700 font-medium">
                              Key Result {index + 1}:
                            </Label>
                            <Textarea
                              value={currentKeyResults[index] || (templateKr.targetValue && templateKr.unit 
                                ? `${templateKr.title} (Target: ${templateKr.targetValue} ${templateKr.unit})`
                                : templateKr.title)}
                              onChange={(e) => {
                                const newKeyResults = [...currentKeyResults];
                              ...
                              className="min-h-[60px] text-sm bg-white border-blue-200 focus:border-blue-400 focus:ring-blue-400"
                            />
                          </div>
                        ))}
                        <p className="text-xs text-blue-600">
                          💡 Tip: Sesuaikan target angka dengan kapasitas perusahaan Anda
                        </p>
                      </div>
                    </div>
                  );
