...
export default function InitiativeClosureModal({
  ...
  const [showIncompleteTasksConfirmation, setShowIncompleteTasksConfirmation] =
    useState(false);
  const [pendingFormData, setPendingFormData] =
    useState<ClosureFormData | null>(null);
  ...
  const updateMetricValue = (metricId: string, value: string) => {
    setUpdatedMetrics((prev) =>
      prev.map((metric) =>
        metric.id === metricId ? { ...metric, currentValue: value } : metric,
      ),
    );
  };
  ...
  const updateTaskStatus = (taskId: string, newStatus: string) => {
    setTaskUpdates((prev) =>
      prev.map((task) => (task.id === taskId ? { ...task, newStatus } : task)),
    );
  };
  ...
  const updateDodStatus = (dodId: string, isCompleted: boolean) => {
    setDodUpdates((prev) =>
      prev.map((dod) =>
        dod.id === dodId ? { ...dod, newStatus: isCompleted } : dod,
      ),
    );
  };
  ...
  const closeInitiativeMutation = useMutation({
    mutationFn: async (data: ClosureFormData) => {
      ...
      for (const dod of dodUpdates || []) {
        if (dod.newStatus !== dod.isCompleted) {
          await apiRequest(
            "PATCH",
            `/api/definition-of-done/${dod.id}/toggle`,
            {
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error.message || "Terjadi kesalahan saat menutup inisiatif",
        variant: "destructive",
      });
    },
  });
  ...
  const incompleteTasks = Array.isArray(tasks)
    ...
    : [];
  const incompleteDodItems = Array.isArray(dodItems)
    ? dodItems.filter((dod) => !dod.isCompleted)
    : [];
  ...
  return (
    <>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <CardContent className="pt-6 space-y-4">
                                ...
                                <div className="space-y-2 text-sm">
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-gray-600">
                                        Inisiatif mencapai tujuan yang
                                  ...
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-gray-600">
                                        Inisiatif tidak mencapai target yang
                                        diharapkan
                                      </p>
                                    </div>
                                  </div>
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-gray-600">
                                        Inisiatif perlu dijalankan kembali
                                        dengan penyesuaian
                                      </p>
                                    </div>
                                  </div>
                    ...
                    <div className="space-y-3 border-t border-gray-200 pt-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-sm font-medium text-gray-700 h-8 flex items-center">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-sm font-medium text-gray-700 h-8 flex items-center">
                                {getLearningPrompt(selectedResult)}
                              </FormLabel>
                              <FormControl>
                                <Textarea
                                  placeholder={getLearningPlaceholder(
                                    selectedResult,
                                  )}
                                  {...field}
                                  rows={2}
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <div className="flex items-center gap-2">
                              ...
                              <Popover>
                                ...
                                <PopoverContent className="w-80">
                                  <div className="space-y-2">
                                    <h4 className="font-medium">
                                      Panduan Update Budget
                                    </h4>
                                    <div className="space-y-2 text-sm">
                                      <p>
                                        • Masukkan total budget yang sudah
                                        digunakan selama inisiatif berjalan
                                      </p>
                                      <p>
                                        • Budget ini akan digunakan untuk
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              Budget awal: Rp{" "}
                              {initiative?.budget
                                ? Number(initiative.budget).toLocaleString(
                                    "id-ID",
                                  )
                                : "0"}
                            </div>
                        ...
                        <div className="mb-3 flex items-center gap-2">
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-80">
                              <div className="space-y-2">
                                <h4 className="font-medium">
                                  Panduan Update Metrik Keberhasilan
                                </h4>
                                <div className="space-y-2 text-sm">
                                  <p>
                                    • Masukkan nilai final yang dicapai untuk
                                    setiap metrik
                                  </p>
                                  <p>
                                    • Nilai ini akan dibandingkan dengan target
                                    yang ditetapkan
                                  </p>
                                  <p>
                                  ...
                                  <p>
                                    • Jika tidak ada perubahan, biarkan nilai
                                    saat ini
                                  </p>
                                </div>
                              </div>
                  ...
                  {selectedResult && incompleteDodItems.length > 0 && (
                    <div className="border-t border-gray-200 pt-4">
                      <div className="mb-3 flex items-center gap-2">
                        <h3 className="text-sm font-medium text-gray-700">
                          Update Status Deliverable (Output)
                        </h3>
                        <Popover>
                          <PopoverTrigger asChild>
                            <button
                          ...
                          <PopoverContent className="w-80">
                            <div className="space-y-2">
                              <h4 className="font-medium">
                                Panduan Update Status Deliverable
                              </h4>
                              <div className="space-y-2 text-sm">
                                <p>
                                  • <strong>Selesai:</strong> Deliverable telah
                                  diselesaikan sepenuhnya
                                </p>
                                <p>
                                  • <strong>Belum Selesai:</strong> Deliverable
                                  belum diselesaikan
                                </p>
                                <p>
                                  • Update status ini untuk mencerminkan kondisi
                                  akhir deliverable
                                </p>
                                <p>
                                  • Deliverable yang sudah selesai akan membantu
                                  menentukan tingkat pencapaian inisiatif
                                </p>
                              </div>
                            </div>
                          </PopoverContent>
                      ...
                      <div className="mb-3 flex items-center gap-2">
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-80">
                            <div className="space-y-2">
                              <h4 className="font-medium">
                                Panduan Update Status Task
                              </h4>
                              <div className="space-y-2 text-sm">
                                <p>
                                  • <strong>Selesai:</strong> Task telah
                                  diselesaikan sepenuhnya
                                </p>
                                <p>
                                  • <strong>Dibatalkan:</strong> Task tidak
                                  dilanjutkan karena alasan tertentu
                                </p>
                                <p>
                                  • <strong>Sedang Berjalan:</strong> Task masih
                                  dalam proses pengerjaan
                                </p>
                                <p>
                                  • <strong>Belum Dimulai:</strong> Task belum
                                  dikerjakan
                                </p>
                                <p>
                                  • Update status ini untuk mencerminkan kondisi
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <div className="flex-shrink-0 w-32">
                                <Select
                                  value={
                                    taskUpdates.find((t) => t.id === task.id)
                                      ?.newStatus || task.status
                                  }
                                  onValueChange={(value) =>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Popover>
                                ...
                                <PopoverContent className="w-80">
                                  <div className="space-y-2">
                                    <h4 className="font-medium">
                                      Panduan Catatan Tambahan
                                    </h4>
                                    <div className="space-y-2 text-sm">
                                      <p>
                                        • Tambahkan informasi penting yang belum
                                        tercakup di bagian lain
                                      </p>
                                      <p>
                                        • Catat kendala atau tantangan yang
                                        dihadapi
                                      </p>
                                      <p>
                                        • Dokumentasikan hal-hal yang perlu
                                        diingat untuk inisiatif serupa
                                      </p>
                                      <p>
                                        • Berikan rekomendasi untuk perbaikan di
                                        masa depan
                                      </p>
                                    </div>
                                  </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {closeInitiativeMutation.isPending
                    ? "Menutup..."
                    : "Tutup Inisiatif"}
                </Button>
              </div>
      ...
      <AlertDialog
        open={showIncompleteTasksConfirmation}
        onOpenChange={setShowIncompleteTasksConfirmation}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                Terdapat <strong>{getIncompleteTasksCount()} task</strong> yang
                masih berstatus
                <strong> "Belum Dimulai"</strong> atau{" "}
                <strong>"Sedang Berjalan"</strong>.
              </p>
              <p>
                Apakah Anda yakin ingin menutup inisiatif ini? Task yang belum
                selesai akan tetap memiliki status saat ini.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
