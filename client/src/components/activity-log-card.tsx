...
export default function ActivityLogCard({ objectiveId }: ActivityLogCardProps) {
  ...
  const { data: activities = [], isLoading } = useQuery<ActivityLogEntry[]>({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/objectives/${objectiveId}/activity-log`,
      );
  ...
  const getTypeHint = (type: string) => {
    switch (type) {
      case "key_result_checkin":
        return "Update progress pada ukuran keberhasilan";
      case "initiative":
        return "Update status atau progress rencana/inisiatif";
      case "task":
        return "Update status atau completion tugas";
      default:
        return "Update aktivitas";
    }
  };
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent className="flex flex-col pb-0" style={{ height: 'calc(100% + 4px)' }}>
        {activities.length > 0 ? (
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        className={`w-6 h-6 ${getTypeColor(activity.type)} rounded-full flex items-center justify-center text-white cursor-help`}
                      >
                      ...
                      {activity.value && (
                        <p
                          className={`font-semibold ${
                            activity.type === "key_result_checkin"
                              ? "text-blue-600"
                          ...
                          <div className="w-16 bg-gray-200 rounded-full h-2">
                            <div
                              className={`h-2 rounded-full ${
                                parseFloat(activity.value) >= 100
                                  ? "bg-green-500"
                                  : parseFloat(activity.value) >= 75
                                    ? "bg-blue-500"
                                    : parseFloat(activity.value) >= 50
                                      ? "bg-yellow-500"
                                      : "bg-red-500"
                              }`}
                              style={{
                                width: `${Math.min(100, Math.max(0, parseFloat(activity.value)))}%`,
                              }}
                            ></div>
                          </div>
          ...
          <div className="lg:flex-1 flex flex-col items-center justify-center text-gray-500 py-8">
            ...
            <p className="text-sm">
              Aktivitas akan muncul ketika ada update pada ukuran keberhasilan,
              rencana, atau tugas.
            </p>
          </div>
        )}
